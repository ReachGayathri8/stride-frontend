...
export default function Register() {
  ...
  const [form, setForm] = useState({
    ...
    fitnessGoal:'GENERAL', budgetTier:'MEDIUM', dietPref:'NONE', sex:'MALE',
  });
  ...
  const submit = async e => {
    ...
    try {
      await register({
        ...form,
        age: +form.age, weightKg: +form.weightKg, heightCm: +form.heightCm,
        male: form.sex === 'MALE',
      });
      navigate('/dashboard');
    } catch (err) {
  ...
  const isBeginnerMode = +form.age < 20 || form.fitnessGoal === 'GENERAL';
  ...
  return (
    <div className="auth-page">
      <div className="auth-card auth-card-wide">
        ...
        <p className="auth-sub">Set up your personalised fitness profile</p>
        ...
        <form onSubmit={submit}>
            ...
            <div className="sex-toggle">
              {[['MALE','Male'],['FEMALE','Female'],['OTHER','Other']].map(([v, l]) => (
                <button key={v} type="button"
                  className={`sex-btn ${form.sex === v ? 'active' : ''}`}
                  onClick={() => setForm(p => ({ ...p, sex: v }))}>
                  {l}
                </button>
              ))}
            </div>
          ...
          {isBeginnerMode && (
            <div className="beginner-notice">
              ⭐ <strong>Beginner Mode will be ON</strong> — we will auto-recommend exercises suited for your level. No manual browsing needed.
            </div>
          )}
