...
export const DEFAULT_EXERCISE_RULES = {
  ...
  'bicep curl': {
    ...
    feedback_cues: { good:'Nice curl! Full range of motion', ok:'Get full range — all the way up', poor:'Control the movement — no swinging' },
  },
  lunge: {
    keypoint_rules: {
    ...
    angle_thresholds: [
      ...
      { joint:['left_shoulder','left_hip','left_knee'], lower:80,  upper:105, weight:0.25, cue:'Keep your torso upright' },
  ...
  plank: {
    ...
    feedback_cues: { good:'Perfect plank position!', ok:'Minor hip sag — brace harder', poor:'Reset — focus on a straight body line' },
  },
  'shoulder press': {
    keypoint_rules: {
      rep_counter: { joint:['left_shoulder','left_elbow','left_wrist'], lower_threshold:80, upper_threshold:160 }
    ...
    angle_thresholds: [
      ...
      { joint:['left_hip','left_shoulder','left_elbow'],     lower:65,  upper:105, weight:0.25, cue:"Don't flare elbows too wide" },
    ],
...
// ─── Voice feedback (used by usePoseDetection) ────────────────────────────────
...
export const VOICE_COMMANDS = {
  squat:              { down:'Go down',    up:'Stand up',   start:"Let's squat!",     setDone:'Set done! Rest.',  finish:'Squats complete. Excellent!' },
  'push up':          { down:'Down',       up:'Push',       start:'Push-ups! Go!',     setDone:'Set logged.',      finish:'Push-ups done. Amazing!' },
  'bicep curl':       { down:'Lower',      up:'Curl up',    start:'Curl time!',        setDone:'Set done.',        finish:'Arms done. Great pump!' },
  lunge:              { down:'Lunge down', up:'Back up',    start:'Lunges! Step out.', setDone:'Set done.',        finish:'Lunges complete!' },
  deadlift:           { down:'Hinge',      up:'Drive up',   start:'Deadlift! Hinge.',  setDone:'Set done.',        finish:'Deadlifts done!' },
  plank:              { down:'Hold',       up:'Breathe',    start:'Plank! Hold it.',   setDone:'Time done.',       finish:'Plank complete!' },
  'overhead press':   { down:'Lower',      up:'Press up',   start:'Press it up!',      setDone:'Set done.',        finish:'Pressing done!' },
  'shoulder press':   { down:'Lower',      up:'Press up',   start:'Shoulder press!',   setDone:'Set done.',        finish:'Pressing done!' },
  'barbell row':      { down:'Release',    up:'Row back',   start:'Row time!',         setDone:'Set done.',        finish:'Rows complete!' },
  'pull up':          { down:'Lower',      up:'Pull up',    start:'Pull-ups! Engage!', setDone:'Set done.',        finish:'Pull-ups done!' },
  'lat pulldown':     { down:'Release',    up:'Pull down',  start:'Lat pulldown!',     setDone:'Set done.',        finish:'Done!' },
  'tricep pushdown':  { down:'Release',    up:'Push down',  start:'Tricep time!',      setDone:'Set done.',        finish:'Triceps done!' },
  burpee:             { down:'Plank',      up:'Jump up',    start:'Burpees! Full go!', setDone:'Set done.',        finish:'Burpees done. Legend!' },
  'jumping jack':     { down:'Out',        up:'In',         start:'Jumping jacks!',    setDone:'Set done.',        finish:'Done!' },
  'mountain climber': { down:'Drive',      up:'Switch',     start:'Climbers! Go!',     setDone:'Set done.',        finish:'Done!' },
  'leg press':        { down:'Lower',      up:'Press',      start:'Leg press!',        setDone:'Set done.',        finish:'Done!' },
  'kettlebell swing': { down:'Hip hinge',  up:'Explode',    start:'Swings! Hinge!',    setDone:'Set done.',        finish:'Swings done!' },
};

export function getVoiceCommands(exerciseName) {
  const key = (exerciseName || '').toLowerCase().trim();
  if (VOICE_COMMANDS[key]) return VOICE_COMMANDS[key];
  const partial = Object.keys(VOICE_COMMANDS).find(k => key.includes(k) || k.includes(key));
  return partial ? VOICE_COMMANDS[partial] : { down:'Down', up:'Up', start:"Let's go!", setDone:'Set done!', finish:'Workout complete!' };
}